...
export const OrderCard = ({
  ...
  return (
    <div
      className={`rounded-[1.25rem] border bg-(--theme-card-bg) transition-all duration-200 ${isSuccess
    ...
    >
      <div className="flex flex-wrap items-center gap-3 p-4">
        <div className="flex-1 min-w-0">
          <p
            className="font-800 text-base text-(--theme-text) tracking-tight"
        ...
        <p className="font-700 text-primary) text-base">
          {formatCurrency(order.total)}
        </p>

        <button
          onClick={() => onToggle(order.orderId)}
          className="flex h-8 w-8 items-center justify-center rounded-full border border-(--theme-border) text-(--theme-text) opacity-60 transition hover:opacity-100 hover:bg-(--theme-secondary-bg)"
      ...
      {isExpanded && (
        <div className="border-t border-(--theme-border) px-1 pb-4">
          <table className="w-full text-sm mt-1">
            <thead>
              <tr className="text-xs text-(--theme-text) opacity-40 uppercase tracking-widest">
            ...
            {!itemsLoading && expandedItems.length > 0 && (
              <tfoot>
                <tr className="border-t border-(--theme-border)">
                  <td colSpan={3} className="py-2 pl-4 text-xs font-700 text-(--theme-text) opacity-50 uppercase tracking-widest">
                    Total
                  </td>
